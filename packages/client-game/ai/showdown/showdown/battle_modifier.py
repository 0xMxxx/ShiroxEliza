...
def export_battle_data(battle):
    """Export the current state of the battle to a JSON file."""
    ...
    battle_data = {
        "turn": battle.turn,
        "user": {
            "active": battle.user.active.to_dict() if battle.user.active else None,
        },
        "opponent": {
            "active": battle.opponent.active.to_dict() if battle.opponent.active else None,
            "reserve": [pkmn.to_dict() for pkmn in battle.opponent.reserve],
            "side_conditions": battle.opponent.side_conditions,
        },
        "weather": battle.weather,
    }

    # Convierte todos los conjuntos a listas
    serializable_battle_data = make_serializable(battle_data)

    output_path = os.path.join(os.getcwd(), "battle_state.json")
    with open(output_path, "w") as json_file:
        json.dump(serializable_battle_data, json_file, indent=4)
    logger.info("Exported battle state to JSON.")
...
def update_battle(battle, msg):
    ...
    for i, line in enumerate(msg_lines):
        ...
        if action == 'turn':
            export_battle_data(battle)  # Export battle state after every turn
            return True
    ...
    if action != "request":
        export_battle_data(battle)  # Export battle state after every action
        return battle.force_switch


async def async_update_battle(battle, msg):
    result = update_battle(battle, msg)
    export_battle_data(battle)  # Ensure state is exported for async calls
    return result