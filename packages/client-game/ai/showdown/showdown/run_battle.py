...
def get_eliza_response(battle):
    """
    Envía datos de la batalla a Eliza y devuelve la respuesta.
    """
    from json_eliza import send_to_eliza, prepare_battle_data

    # Prepara el estado actual de la batalla para enviarlo
    battle_data = {
        "turn": battle.turn,
        "user": {
            "active": {
                "id": battle.user.active.name,
                "hp": battle.user.active.hp,
                # Usar "move.move" como fallback si "id" no existe
                "moves": [getattr(move, "id", getattr(move, "move", "unknown")) for move in battle.user.active.moves]
            }
        },
        "opponent": {
            "active": {
                "id": battle.opponent.active.name,
                "hp": battle.opponent.active.hp
            }
        }
    }

    # Reduce el tamaño del JSON para enviarlo a Eliza
    simplified_battle_data = prepare_battle_data(battle_data)

    # Envía el estado de la batalla a Eliza y recibe la respuesta
...
async def start_battle(ps_websocket_client, pokemon_battle_type):
    ...
    # Enviar un mensaje al inicio de la batalla
    await ps_websocket_client.send_message(battle.battle_tag, ["hf"])
    await ps_websocket_client.send_message(battle.battle_tag, ['/timer on'])


    # Enviar un mensaje de Eliza
    eliza_response = get_eliza_response(battle)  # Llamar a la función para obtener el mensaje

    if eliza_response:
    # Extraer el texto de la respuesta
        eliza_text = eliza_response[0]['text']

    # Confirmar que el texto es una cadena
    if isinstance(eliza_text, str):
        print(f"Enviando al chat del juego: {eliza_text}")
        # Enviar el texto directamente al juego como una cadena
        await ps_websocket_client.send_message(battle.battle_tag, [f"{eliza_text}"])
    else:
        # Manejo de errores si eliza_text no es una cadena
            print("Error: Eliza respondió con un formato incorrecto, texto no es una cadena.")
    return battle
...
async def pokemon_battle(ps_websocket_client, pokemon_battle_type):
    ...
    while True:
        ...
        # Si se requiere acción (como elegir un movimiento)
        if action_required and not battle.wait:
            # Cada 2 turnos, enviar mensaje de Eliza al chat s
