...
export class ClientBase extends EventEmitter {
    ...
    async fetchUserTweets(
        ...
        try {
            // Usamos 'from:username' para obtener los tweets del usuario
            const query = `from:${username}`;
    
            // Reutilizamos fetchSearchTweets para obtener los tweets
            const result = await this.fetchSearchTweets(
                query,
                maxTweets,
                SearchMode.Latest,
                cursor
    ...
    async setCookiesFromArray(cookiesArray: any) {
        // Verificamos si el array de cookies está en `cookiesArray.cookies`
        ...
        if (!Array.isArray(actualCookiesArray)) {
            console.error("cookiesArray no es un array:", actualCookiesArray);
            return;
        }
